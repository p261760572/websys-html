...
(function (xTool) {
    ...
    function send(url, data, success, async) {
    	...
    	if(query.displayType !== undefined && data.displayType === undefined) {
    		data.displayType = query.displayType;
    	}
        $.ajax(getUrl(url), {
            type: 'POST',
	...
	*/
    ...
    function selectRow(rowIndex, rowData) {
        ...
        var tool = $('>.datagrid-body>.datagrid-toolbar', t);
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        ...
        maxByteLength: {
            validator: function (value, param) {
                ...
                return byteLength(value) <= param[0];
            },
            message: '输入内容最多为{0}个字符,1个汉字占2个字符'
        },
        accountNo: {
            validator: function (value, param) {
                var re = /^[0-9\-]+$/;
    ...
    function tabs() {
        $('.easyui-tabs').each(function(i, domEle) {
            ...
            var panels = t.children('div').each(function(i) {
                ...
                if (o.closable) {
                    s.push('<a class="tabs-close" href="javascript:void(0)">×</a>');
                }
                s.push('</li>');
                nav.append(s.join(''));
