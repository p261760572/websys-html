...
(function($) {
    ...
    function init(target) {
		...
		$('.combobox-text', target).validatebox($.extend({
    ...
    function setSize(target) {
        ...
        t.children('.combobox-text')._outerWidth(t.width()-addonWidth);
    }
    ...
    function bindEvents(target) {
        ...
        var t = $(target);
        var input = t.children('.combobox-text');
        var arrow = t.children('.combobox-addon');
    ...
    function showPanel(target) {
        ...
        function calcLeft() {
            ...
            if (left + t.outerWidth() > $(window).outerWidth() + $(document).scrollLeft()) {
                left = $(window).outerWidth() + $(document).scrollLeft() - t.outerWidth();
            }
            if (left < 0) {
                left = 0;
            }
        ...
        function calcTop() {
            ...
            if (top + opts.panelHeight > $(window).outerHeight() + $(document).scrollTop()) {
                top = t.offset().top - opts.panelHeight;
            }
    ...
    function setDisabled(target, disabled) {
        ...
        if (disabled) {
            opts.disabled = true;
            t.children('input.combobox-text').attr('disabled', 'disabled');
        } else {
            opts.disabled = false;
            t.children('input.combobox-text').removeAttr('disabled');
        }
    }
    ...
    function getValue(target) {
        ...
        var t = $(target);
        return t.children('input.combobox-value').val();
    }
    ...
    function setValue(target, value, remainText) {
        ...
        //值
        t.children('input.combobox-value').val(value);
        //文本
        if(!remainText) {
        	t.children('input.combobox-text').val(text);
    	}
        
        if(oldValue != value) {
       		opts.onChange.call(target, value, oldValue);
       	}
       	$('.combobox-text', target).validatebox('validate');
    }
    
    function clear(target, value) {
        var state = $.data(target, 'combobox');
        var panel = state.panel;
        state.previousValue = undefined; //清空query
        
        panel.children('li.combobox-item').removeClass('combobox-item-selected').show();
        $(target).children('input.combobox-value,input.combobox-text').val('');
    }
...
(function($) {
    ...
    function load(target, data) {
        ...
        function _load(data) {
            ...
            for (var name in data) {
                ...
                if ($.fn.combobox) {
                    $('input[name="' + name + '"]', form).closest('.combobox').combobox('setValue', val);
                }
            }
        }
    }
    ...
    function clear(target) {
        ...
        if ($.fn.combobox) {
            $('.combobox', target).combobox('clear');
        }
    }
	...
	function setEditable(target, editable){
		if(editable) {
			...
			$('.combobox', target).combobox('enable');
			//data-editable
...
(function($) {
    ...
    function wrapGrid(target) {
    	var table = $('.datagrid-table', target);
    	var pager = $('.datagrid-pager', target);
	...
	function buildToolbar(target) {
        $('.datagrid-toolbar', target).children('.l-btn').linkbutton();
    }
    ...
    $.fn.datagrid = function(options, param) {
        ...
        return this.each(function() {
            ...
            if (state) {
                ...
                $.data(this, 'datagrid', {
                	table: wrapResult.table,
                	pager: wrapResult.pager,
                    options: opts,
                    data: {
                        total: 0,
                        rows: []
...
(function($) {
    ...
    function bindEvents(target) {
        ...
        var titles = $('>div.tabs-header>ul>li', t);
        var panels = $('>div.tabs-panels>div', t);
        state.titles = titles;
        state.panels = panels;
...
(function($) {
    ...
    function init(target) {
        ...
        $('>div.dialog-view>div.dialog-toolbar>a.l-btn', target).linkbutton();
        $('>div.dialog-view>div.dialog-button>a.l-btn', target).linkbutton();
    }
    ...
    function bindEvents(target) {
        ...
        $('>div.dialog-header>.dialog-close', target).unbind('.dialog').bind('click.dialog', function() {
            close(target);
        });

		if(opts.modal) {
	        t.unbind('.dialog').bind('_resize.dialog', function() {
	            if (t.is(':visible')) {
	                setSize(target);
	                center(target);
	            }
